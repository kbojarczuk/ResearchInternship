...
for i in range(0,7):
    year = data[3][2+i*4]
    sth = sh.col_values(0,18,20)
    prg = sh.col_values(2+i*4,18,20)
    rg = sh.col_values(3+i*4,18,20)
    resp = sh.col_values(4+i*4,18,20)
...
for i in range(0,3):
    year = data[3][30+i*14]
    sth = sh.col_values(0,18,20)
    prg = sh.col_values(30+i*14,18,20)
    rg = sh.col_values(32+i*14,18,20)
    resp = sh.col_values(42+i*14,18,20)
...
with open('tp_arts_ns_m.csv', 'w', newline='') as f:
    c = csv.writer(f)
    for i in list:
        c.writerow(i)
