...
def write(filename,sh):
    ...
    for i in range(11, 13):
        ...
        empty = "\\N"
        #subsector stats
        distribution = [sh1.cell_value(6,c) for c in range(1,4)]
        type = sh1.cell_value(6,0)
        stats = [sh1.cell_value(5,c) for c in range(1,4)]
...
with open('blueprint10type.csv', 'w', newline='') as f:
    c = csv.writer(f)
    for i in list:
        c.writerow(i)
